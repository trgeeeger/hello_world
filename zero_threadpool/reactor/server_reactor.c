...
#define NOSET_CB	0  //这个宏表示event没有在epoll里面注册
#define READ_CB		1
#define WRITE_CB	2
#define ACCEPT_CB	3
/*单例模式*/
...
struct reactor *getInstance(void) { //singleton 该函数的作用就是创建一个reactor结构体指针
	if (instance == NULL) {
...
int nreactor_set_event(int fd, NCALLBACK cb, int event, void *arg) {
	...
	//以下三个实例中，items相当于一个结构体指针字符串，运用了一个使用fd为下标的类似hash的索引
	//以下三个示例都是对于事件的初始化
	if (event == READ_CB) {
	...
	/*NOSET_CB 0*/
	if (r->head->items[fd].events == NOSET_CB) {					//这段代码的含义是如果某个事件还没有注册到epoll事件里面时（event创建时默认就是0）
		if (epoll_ctl(r->epfd, EPOLL_CTL_ADD, fd, &ev) < 0) {		//就将这个时间==事件注册到epoll事件里面去
			printf("epoll_ctl EPOLL_CTL_ADD failed, %d\n", errno);
			return -1;
		}
		r->head->items[fd].events = event;
	} else if (r->head->items[fd].events != event) {			//这段代码意思是对于某一个fd来说，不仅仅只有一个事件，也可以有多个，														
		if (epoll_ctl(r->epfd, EPOLL_CTL_MOD, fd, &ev) < 0) {	//所以这段代码判断与传入的事件不同时就修改epoll里面注册的fd事件类型
...
int reactor_loop(int listenfd) {
	...
	while (1) {
		...
		for (i = 0;i < nready;i ++) {
			...
			struct nitem *item = (struct nitem *)events[i].data.ptr;//在这行代码里面在epoll_ctl传递的是什么类型的数据
			int connfd = item->fd;									//在epoll_wait传出的就是什么类型数据
			if (connfd == listenfd) { //如果条件成立表示某个fd想要建立连接
				item->acceptcb(listenfd, 0, NULL);
			} else {
			
				if (events[i].events & EPOLLIN) { //如果条件成立表示有数据需要读取
					item->readcb(connfd, 0, NULL);
				
				} 
				if (events[i].events & EPOLLOUT) {//如果条件成立表示有数据可以写入
...
int main(int argc, char **argv) 
{
 	...
 	int  connfd, n;
	int listenfd = init_server(10090);
	nreactor_set_event(listenfd, accept_callback, ACCEPT_CB, NULL);
