...
struct eventblock {             //一个块的大小，每个块有1024字节
	
	struct eventblock *next;    //next指针
	struct ntyevent *events;    //每个块里面的事件的头指针，指向的是块里面的每一个事件
};
...
struct ntyreactor {
	...
	struct eventblock *evblks;  //块的头指针
};
...
int init_sock(short port) {     //初始化监听端口
...
int ntyreactor_alloc(struct ntyreactor *reactor) {//增加块的数量
	...
	struct ntyevent* evs = (struct ntyevent*)malloc((MAX_EPOLL_EVENTS) * sizeof(struct ntyevent));//创建1024个事件为一个块
	if (evs == NULL) {
	...
	struct eventblock *block = malloc(sizeof(struct eventblock));//创建一个块的对象
	if (block == NULL) {
...
struct ntyevent *ntyreactor_idx(struct ntyreactor *reactor, int sockfd) {//找出sockfd在具体哪个块的哪个地址上的那个事件

	if (reactor == NULL) return NULL;
	if (reactor->evblks == NULL) return NULL;

	int blkidx = sockfd / MAX_EPOLL_EVENTS; //找出是第几个快
	while (blkidx >= reactor->blkcnt) {     //将这个fd本该在的块的内存以及前面的块的内存创建出来
...
int ntyreactor_init(struct ntyreactor *reactor) {//ntyreactor对象初始化
	...
	struct ntyevent* evs = (struct ntyevent*)malloc((MAX_EPOLL_EVENTS) * sizeof(struct ntyevent));//创建一个1024个事件，每1024个事件为一个块
	if (evs == NULL) {
		printf("create epfd in %s err %s\n", __func__, strerror(errno));
		close(reactor->epfd);
		return -3;
	...
	struct eventblock *block = malloc(sizeof(struct eventblock));//创建块
	if (block == NULL) {
		free(evs);
		close(reactor->epfd);
		return -3;
...
int ntyreactor_addlistener(struct ntyreactor *reactor, int sockfd, NCALLBACK *acceptor) {
	...
	struct ntyevent *event = ntyreactor_idx(reactor, sockfd);//找出属于socket的事件
	if (event == NULL) return -1;
...
int main(int argc, char *argv[]) {
	...
	struct ntyreactor *reactor = (struct ntyreactor*)malloc(sizeof(struct ntyreactor));//单例模式的对象
	ntyreactor_init(reactor);   //reactor对象初始化

	unsigned short port = SERVER_PORT;  //端口号
	if (argc == 2) {            //如果有参数，就重新设置端口号
		port = atoi(argv[1]);
	}
